...
import { setFilterData, getPageData, setPageData, clearPageArr, changePageArr } from '../../redux/actions/actions'
  ...
  return (
    <Form className="mb-4" onSubmit={(event) => {
      ...
      <Form.Group controlId="formBasicEmail">
        ...
        <Form.Control type="text" placeholder="Enter search data" value={serachValue} onChange={(event) => {
          setSearchValue(event.target.value)
          let newData = textFilter(allData, event.target.value)
...
const mapDispatchToProps = dispatch => {
  return {
    changeFilterData: (data) => {
      ...
      let maxPageIndex = Math.ceil(data.length / 50)
    ...
    changePageData: (data) => { dispatch(setPageData(data)) }
  }
}
